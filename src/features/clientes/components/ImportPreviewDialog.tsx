...
interface ImportPreviewDialogProps {
  ...
  importing: boolean;
  /** Muestra la columna Empresa (uso en el listado global de Clientes). */
...
const STATUS_BADGE: Record<ImportRowStatus, { label: string; className: string; icon: typeof CheckCircle2 }> = {
  ...
  invalid: { label: 'Inválido', className: 'bg-red-50 text-red-700 border-red-200', icon: CircleAlert },
};

export function ImportPreviewDialog({ open, fileName, rows, importing, showEmpresaColumn, onCancel, onConfirm }: ImportPreviewDialogProps) {
      ...
      <DialogContent className="sm:max-w-[760px] max-h-[85vh] flex flex-col">
              ...
              {rows.map((row) => {
                ...
                return (
                  <TableRow key={row.rowNum} className={row.status !== 'ok' ? 'opacity-70' : undefined}>
                    ...
                    <TableCell className="tabular-nums">{row.celular || '—'}</TableCell>
                    <TableCell>{row.cargo || '—'}</TableCell>
                    <TableCell className="tabular-nums">{row.cedula || '—'}</TableCell>
                    {showEmpresaColumn && <TableCell>{row.empresaNombre || '—'}</TableCell>}
                    <TableCell>
        ...
        <DialogFooter className="shrink-0">
          ...
          <Button type="button" onClick={onConfirm} disabled={importing || okCount === 0}>
            ...
            {importing ? 'Importando...' : `Importar ${okCount} cliente${okCount !== 1 ? 's' : ''}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
